...
说明：真实电力 API 接入前，这些表由 scripts/seed_mock_data.py 生成模拟数据，
支撑指标查询与异常分析闭环。字段按电力营销常见口径命名。

设计决策：
- 按"区域 + 日期"为粒度：支撑按区域/日期聚合的典型分析问题。
- 金额与电量统一用 Decimal（避免浮点误差），不做 float。
"""
...
class RegionDailyMetric(Base):
    """区域日度营销指标：供电量、售电量、线损等（按区域+日期一行）。"""

    __tablename__ = "region_daily_metrics"

    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    region: Mapped[str] = mapped_column(String(64), index=True)  # 区域名称
    stat_date: Mapped[date] = mapped_column(Date, index=True)  # 统计日期
    ...
    collection_rate: Mapped[Decimal | None] = mapped_column(Numeric(10, 6), nullable=True)  # 电费回收率
